...
export const VideoPreview = () => {
  ...
  }, [setCurrentTime, setIsPlaying, isDraggingText]);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      updateMediaItem(activeTextItem, {
        position: {
          x,
          y
        }
  ...
  }, [isDraggingText, activeTextItem, updateMediaItem, setIsDraggingText]);
  // Get the main video if available
  const mainVideo = activeVideoItems[0];
  return <div className="flex-1 flex flex-col p-4 min-h-0">
      <div ref={previewRef} className="relative flex-1 bg-black rounded-lg flex items-center justify-center overflow-hidden">
        {/* Video element */}
        {mainVideo ? <video ref={videoRef} src={mainVideo.url} className="max-w-full max-h-full object-contain" muted={mainVideo.isMuted} loop /> : <div className="absolute inset-0 flex items-center justify-center bg-gray-800 text-white">
            No video selected
          </div>}
        {/* Text overlays */}
        {activeTextItems.map(item => <div key={item.id} className={`absolute text-white cursor-move ${activeTextItem === item.id ? 'ring-2 ring-blue-500' : ''}`} style={{
