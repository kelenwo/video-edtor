...
export const Sidebar = () => {
  ...
  const updateSelectedText = () => {
    if (selectedTextItem) {
      updateMediaItem(selectedTextItem.id, {
        content: textContent,
        fontSize,
        fontFamily,
        fontColor: textColor,
  ...
  return <div className="flex h-full">
      ...
      <div className="w-16 bg-gray-900 flex flex-col items-center py-4">
        ...
        <div className="flex flex-col space-y-6 items-center">
          <button className={`p-2 rounded-lg ${activeItem === 'add' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('add')}>
            <PlusIcon size={20} />
            <span className="text-xs mt-1 block">Add</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'uploads' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('uploads')}>
            <UploadIcon size={20} />
            <span className="text-xs mt-1 block">Uploads</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'text' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('text')}>
            <TextIcon size={20} />
            <span className="text-xs mt-1 block">Text</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'videos' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('videos')}>
            <VideoIcon size={20} />
            <span className="text-xs mt-1 block">Videos</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'audios' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('audios')}>
            <MusicIcon size={20} />
            <span className="text-xs mt-1 block">Audios</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'photos' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('photos')}>
            <ImageIcon size={20} />
            <span className="text-xs mt-1 block">Photos</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'records' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('records')}>
            <ClockIcon size={20} />
            <span className="text-xs mt-1 block">Records</span>
          </button>
          <button className={`p-2 rounded-lg ${activeItem === 'subtitles' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`} onClick={() => setActiveItem('subtitles')}>
      ...
      {sidebarOpen && <div className="w-64 bg-gray-800 text-white flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {activeItem === 'add' && <div className="space-y-4">
            ...
            {activeItem === 'text' && <div className="space-y-4">
                ...
                {selectedTextItem ? <button onClick={updateSelectedText} className="w-full py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700">
                    Update Text
                  </button> : <button onClick={handleAddText} className="w-full py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700">
                    Add Text
                  </button>}
              </div>}

            {activeItem === 'videos' && <div className="space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    {mediaItems.filter(item => item.type === 'video').map(video => <div key={video.id} className={`p-2 rounded-md cursor-pointer ${selectedItemId === video.id ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`} onClick={() => setSelectedItemId(video.id)}>
                          <div className="flex items-center">
                    ...
                    {mediaItems.filter(item => item.type === 'video').length === 0 && <div className="text-center py-4 text-gray-500 text-sm">
                        No videos added yet
                      </div>}
                  </div>
                </div>
            ...
            {activeItem === 'audios' && <div className="space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    {mediaItems.filter(item => item.type === 'audio').map(audio => <div key={audio.id} className={`p-2 rounded-md cursor-pointer ${selectedItemId === audio.id ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`} onClick={() => setSelectedItemId(audio.id)}>
                          <div className="flex items-center">
                    ...
                    {mediaItems.filter(item => item.type === 'audio').length === 0 && <div className="text-center py-4 text-gray-500 text-sm">
            ...
            {activeItem === 'photos' && <div className="space-y-4">
                ...
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {mediaItems.filter(item => item.type === 'image').map(image => <div key={image.id} className={`aspect-square rounded-md overflow-hidden cursor-pointer ${selectedItemId === image.id ? 'ring-2 ring-blue-500' : ''}`} onClick={() => setSelectedItemId(image.id)}>
                          <img src={image.url} alt={image.name} className="w-full h-full object-cover" />
                        </div>)}
                  </div>
                  {mediaItems.filter(item => item.type === 'image').length === 0 && <div className="text-center py-4 text-gray-500 text-sm">
                      No photos added yet
                    </div>}
                </div>
              </div>}

            {activeItem === 'records' && <div className="space-y-4">
            ...
            {activeItem === 'subtitles' && <div className="space-y-4">
                <div className="text-center py-4">
            ...
            {activeItem === 'uploads' && <div className="space-y-4">
                <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center hover:border-blue-500 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  <UploadIcon size={24} className="mx-auto mb-2 text-gray-400" />
                  <p className="text-sm text-gray-300">
                    Drag & drop or click to upload
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2">
                    {mediaItems.slice(0, 5).map(item => <div key={item.id} className={`p-2 rounded-md cursor-pointer ${selectedItemId === item.id ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`} onClick={() => setSelectedItemId(item.id)}>
                        <div className="flex items-center">
                    ...
                    {mediaItems.length === 0 && <div className="text-center py-4 text-gray-500 text-sm">
                        No uploads yet
                      </div>}
                  </div>
                </div>
      ...
      {!sidebarOpen && <button onClick={toggleSidebar} className="p-2 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-r-md">
